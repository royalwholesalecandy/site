        ...
        if (in_array($fullActionName, ['catalog_product_view', 'catalog_category_view', 'checkout_cart_index'])) {
            ...
            if ($type !== false) {
                ...
                if ($this->_abstractDeal->isEnableRandomDeal()) {
                    $randomHtml .= $layout->createBlock('Mageplaza\DailyDeal\Block\Widget\RandomDeal')
                        ->setTemplate('Mageplaza_DailyDeal::widget/sidebar-widget.phtml')->toHtml();
                }

                if ($this->_abstractDeal->isEnableSellingDeal()) {
                    $sellingHtml .= $layout->createBlock('Mageplaza\DailyDeal\Block\Widget\TopSellingDeal')
                        ->setTemplate('Mageplaza_DailyDeal::widget/sidebar-widget.phtml')->toHtml();
                }

                if ($this->_abstractDeal->isEnableUpcomingDeal()) {
                    $upcomingHtml .= $layout->createBlock('Mageplaza\DailyDeal\Block\Widget\UpcomingDeal')
                        ->setTemplate('Mageplaza_DailyDeal::widget/sidebar-widget.phtml')->toHtml();
                }

                if ($this->_abstractDeal->getRandomShowOn() == WidgetShowOn::SIDEBAR_LEFT) {
                    $sidebarTopHtml .= $randomHtml;
                } else {
                    $sidebarBottomHtml .= $randomHtml;
                }

                if ($this->_abstractDeal->getSellingShowOn() == WidgetShowOn::SIDEBAR_LEFT) {
                    $sidebarTopHtml .= $sellingHtml;
                } else {
                    $sidebarBottomHtml .= $sellingHtml;
                }

                if ($this->_abstractDeal->getUpcomingShowOn() == WidgetShowOn::SIDEBAR_LEFT) {
