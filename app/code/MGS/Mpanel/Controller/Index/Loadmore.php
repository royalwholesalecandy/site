    ...
    public function execute()
    {
		...
		switch ($type) {
			...
			case 'new-products-category':
				...
				if($html != $htmlNext){
					$result['url'] = $this->_urlBuilder->getUrl('mpanel/index/loadmore', ['type'=>$type, 'category'=>$categoryId, 'limit'=>$limit, 'p'=>$nextPage, 'ratio'=>$ratio, 'col'=>$col, 'block_id'=>$blockId, 'tabs'=>$tabs, 'perrow' => $perrow]);
			...
			case 'attribute-products-category':
				...
				if($html != $htmlNext){
					$result['url'] = $this->_urlBuilder->getUrl('mpanel/index/loadmore', ['type'=>$type, 'category'=>$categoryId, 'limit'=>$limit, 'p'=>$nextPage, 'ratio'=>$ratio, 'col'=>$col, 'block_id'=>$blockId, 'attribute'=>$attribute, 'tabs'=>$tabs, 'perrow'=> $perrow]);
					$result['next'] = $htmlNext;
				}else{
					if($htmlPrev == $html){
						$html = '';
