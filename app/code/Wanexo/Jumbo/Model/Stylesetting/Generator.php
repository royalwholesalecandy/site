...
namespace Wanexo\Jumbo\Model\Stylesetting;

class Generator
{
    protected $_messageManager;
    protected $_stylesettingData;
    protected $_coreRegistry;
    protected $_storeManager;
    protected $_layoutManager;
    
    public function __construct(
        \Wanexo\Jumbo\Helper\Stylesetting $stylesettingData,
        \Magento\Framework\Registry $coreRegistry,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \Magento\Framework\View\LayoutInterface $layoutManager,
        \Magento\Framework\Message\ManagerInterface $messageManager
    ) {
        $this->_stylesettingData = $stylesettingData;
    ...
    protected function generateStoreCss($type, $storeId) {
        ...
        $str1 = '_'.$storeCode;
        $str2 = $type.$str1.'.css';
        $str3 = $this->_stylesettingData->getStylesettingDir().$str2;
        $str4 = 'jumbo/css/'.$type.'.phtml';
        $this->_coreRegistry->register('cssgen_store', $storeCode);

        try {
            $block = $this->_layoutManager->createBlock('Wanexo\Jumbo\Block\Template')->setData('area','frontend')->setTemplate($str4)->toHtml();
            if(!file_exists($this->_stylesettingData->getStylesettingDir())) {
                @mkdir($this->_stylesettingData->getStylesettingDir(), 0777);
            }
            $file = @fopen($str3,"w+");
            @flock($file, LOCK_EX);
            @fwrite($file,$block);
            ...
            $this->_messageManager->addError(__('Failed generating CSS file: '.$str2.' in '.$this->_stylesettingData->getStylesettingDir()).'<br/>Message: '.$e->getMessage());
        }
