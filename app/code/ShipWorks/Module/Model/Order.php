...
class Order implements OrderInterface
{
    ...
    // Instance of \Magento\Sales\Api\ShipOrderInterface
    protected $ShipOrder;
    ...
    private function createMissingOrderSearchCriteria($date, $excludeOrders)
    {
        ...
        $searchCriteria->setFilterGroups(array($this->createFilterGroup(array($dateFilter)), $this->createFilterGroup(array($idFilter))));
    ...
    private function createOrderSearchCriteria($date, $maxcount = 0)
    {
        ...
        $searchCriteria->setFilterGroups(array($this->createFilterGroup(array($filter))))
            ->setSortOrders($this->createSortOrder('updated_at', 'asc'));
        ...
        try {
            ...
            switch ($action) {
                case self::COMPLETE:
                    ...
                    $this->shipOrder($order, $this->OrderActionParameters);
     ...
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    private function shipOrder($order, $params)
    {
        ...
        // If we can ship the order we ship it
        if ($order->canShip()) {
     ...
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    private function addTracking($order, $track)
    {
