...
define([
    ...
    'form',
    'validation'
], function (jQuery, mageTemplate, mage, rg) {
    'use strict';
    jQuery('#edit_form').append(jQuery('#variable_map_block'));
    jQuery('#variable_map_block').attr('style', 'display: block;');
    return function (config) {
        var attributeOption = {
                ...
                totalItems: 0,
                rendered: 0,
                template: mageTemplate('#variable-row-template'),
        ...
        jQuery('#manage-options-panel').on('keyup', '.ebay_listing_var', function () {
            jQuery(this).val(jQuery(this).val().replace(/[^A-Z0-9]/ig, ""));
        });
        attributeOption.setItemCount(config.attributesData);
