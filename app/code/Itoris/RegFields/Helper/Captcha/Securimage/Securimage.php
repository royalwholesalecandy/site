...
/**
 ...
 - Allow HTML color codes to be passed to Securimagecolor (suggested by Mike Challis)

 2.0.0
 - Add mathematical distortion to characters (using code from HKCaptcha)
 - Improved session support
 - Added Securimagecolor class for easier color definitions
...
class Securimage {
    ...
    /**
     * The background color for the image as a Securimagecolor.<br />
     *
     * @var Securimagecolor
     */
    var $image_bg_color;
    ...
    /**
     * The text color to use for drawing characters as a Securimagecolor.<br />
    ...
    /**
     * Array of Securimagecolors which will be randomly selected for each letter.<br />
     *
     * @var array
     */
    ...
    */
    ...
    function __construct()
    {
        ...
        $this->image_bg_color   = new Securimagecolor(0xff, 0xff, 0xff);
        $this->text_color       = new Securimagecolor(0x3d, 0x3d, 0x3d);
        $this->multi_text_color = [new Securimagecolor(0x0, 0x20, 0xCC),
                                        new Securimagecolor(0x0, 0x30, 0xEE),
                                        new Securimagecolor(0x0, 0x40, 0xCC),
                                        new Securimagecolor(0x0, 0x50, 0xEE),
                                        new Securimagecolor(0x0, 0x60, 0xCC)];
        ...
        $this->num_lines            = 10;
        $this->line_color           = new Securimagecolor(0x3d, 0x3d, 0x3d);
        $this->draw_lines_over_text = true;

        $this->image_signature = '';
        $this->signature_color = new Securimagecolor(0x20, 0x50, 0xCC);
    ...
    function saveData()
    {
        ...
        $session->setSecurimageCodeValue(strtolower($this->code));
        $session->setSecurimageCodeCtime(time());
    ...
    function validate()
    {
        ...
        if (trim($session->getSecurimageCodeValue()) != '') {
            //if ($this->isCodeExpired($_SESSION['securimage_code_ctime']) == false) { 
            if ($this->isCodeExpired($session->getSecurimageCodeCtime()) == false) { 
              //$code = $_SESSION['securimage_code_value'];
              $code = $session->getSecurimageCodeValue();
            }
        ...
        if ($code != '') {
              ...
              $session->setSecurimageCodeValue('');
              $session->setSecurimageCodeCtime('');
    ...
    function getCode()
    {
        ...
        if (!empty($session->getSecurimageCodeValue())) {
            //return strtolower($_SESSION['securimage_code_value']);
            return strtolower($session->getSecurimageCodeValue());
        } else {
